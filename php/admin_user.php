<?php
// admin_user.php - Admin kullanıcı yönetimi sayfası
...
try {
    ...
    $stmt_users = $conn->prepare("SELECT id, username, email, role FROM users ORDER BY id DESC");
    $stmt_users->execute();
...
<head>
    ...
    <style>
        ...
        .users-table { width: 100%; border-collapse: collapse; margin-top: 20px; font-size: 0.95rem; }
        .users-table th, .users-table td { padding: 12px 15px; text-align: left; border-bottom: 1px solid #dee2e6; vertical-align: middle; }
        .users-table th { background-color: #e9ecef; font-weight: 600; color: #495057; }
        .users-table tbody tr:hover { background-color: #f8f9fa; }
        .btn-action { padding: 0.35rem 0.7rem; font-size: 0.9rem; margin-right: 6px; border-radius: 0.25rem; }
        ...
        .status-badge { padding: 0.4em 0.7em; font-size: 0.85em; font-weight: 500; border-radius: 0.3rem; display: inline-block; min-width: 90px; text-align: center; }
        .role-customer { background-color: #cfe2ff; color: #052c65; }
        .role-seller { background-color: #d2f4ea; color: #0a3622; }
        .role-admin { background-color: #fdebd0; color: #593202; }
                    ...
                    <td>
                        ...
                        try {
                            if ($user['role'] === 'seller') {
                                ...
                                if ($seller_data) {
                                    if ($seller_data['HesapDurumu'] == 1) { $user_status = 'Aktif'; $status_class = 'status-aktif'; }
                                    else { $user_status = 'Pasif/Doğrulanmamış'; $status_class = 'status-pasif'; }
                                } else { $user_status = 'Satıcı Kaydı Yok'; $status_class = 'status-satici-bilgisi-eksik'; }
                                ...
                                if ($customer_data) {
                                    if($customer_data['Uyelik_Durumu'] == 1){ $user_status = 'Aktif'; $status_class = 'status-aktif'; }
                                    else { $user_status = 'Pasif'; $status_class = 'status-pasif'; }
                                } else { $user_status = 'Müşteri Kaydı Yok'; $status_class = 'status-satici-bilgisi-eksik'; }
